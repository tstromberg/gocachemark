...
type MemoryResult struct {
	...
	Items           int
	Bytes           uint64
	BytesPerItem    int64 // overhead vs baseline
...
func RunMemory(capacity, valSize int) ([]MemoryResult, error) {
	...
	buildCmd := exec.Command("go", "build", "-o", binPath, "./cmd/mem")
	...
	for i := range results {
		...
		if results[i].Items > 0 {
			diff := int64(results[i].Bytes) - int64(baseline.Bytes)
